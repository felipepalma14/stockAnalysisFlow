...
class StocksTypeAdapter(
    private val context:Context,
    private val list: List<String>,
    private val recyclerView: RecyclerView,
    private val listener: (String) -> Unit
) : RecyclerView.Adapter<StocksTypeAdapter.ViewHolder>() {
    private var selectedItem: Int = 0
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_stocks_type_adapter, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if(selectedItem == holder.adapterPosition){
            setItemActivate(holder)
        }
        holder.container.setOnClickListener {
            val view = recyclerView.findViewHolderForAdapterPosition(selectedItem)?.itemView
            view?.let {
                setItemInactivate(ViewHolder(it))
            }
